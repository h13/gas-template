...
│       └── sync-template.yml # テンプレートリポからの更新同期
...
├── .templatesyncignore     # テンプレート同期の除外ファイル
...
このテンプレートには、上流テンプレートリポジトリからの更新を毎週自動チェックする GitHub Actions ワークフロー（`.github/workflows/sync-template.yml`）が含まれています。

更新が見つかると `template-sync` ラベル付きの PR が作成されます。内容を確認してマージしてください。Actions タブから手動で同期を実行することもできます。

プロジェクト固有のファイル（`src/`、`test/`、`README.md` 等）は `.templatesyncignore` で同期対象から除外されているため、コードが上書きされることはありません。同期されるのはツールや設定ファイルの更新のみです。

> **注意**: このワークフローはテンプレートから作成されたリポジトリでのみ実行されます。テンプレートリポジトリ自体では実行されません。
